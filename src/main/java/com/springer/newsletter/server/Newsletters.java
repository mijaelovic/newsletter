...
public class Newsletters {
    ...
    public List<Newsletter> getNewsletter() {
        ...
        try {
            List<Newsletter> ret = new ArrayList<>();
            ...
            for (Object item: query.getResultList()) {
                ...
                Set<Long> visitedBooks = new HashSet<>();
                ...
                while (!queue.isEmpty())
                {
                    ...
                    for (com.springer.newsletter.model.Book book: category.getBooks())
                        if (!visitedBooks.contains(book.getId())){
                            visitedBooks.add(book.getId());
                    ...
                    for (com.springer.newsletter.model.Category child: category.getChildrenCategory())
                        if (!visitedCategoryCodes.contains(child.getCode())) {
                            visitedCategoryCodes.add(child.getCode());
                            queue.add(child);
                        }
                }
                ret.add(new Newsletter(
